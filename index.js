...
function findIndexByProperty(arr, name) {
  ...
  const findElement = (element) => element.name === name;
...
function findLastEvenNumber(arr) {
  ...
  // const findElement = arr.findLast((elem) => elem % 2 === 0);
  // не та версія

  const findElement = arr.filter((elem) => elem % 2 === 0);

  if (findElement.length === 0) {
    return "Парних чисел не знайдено";
  }

  return findElement.pop();
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const arrSlice = arr.slice(startIdx, 6);
...
function getPositiveArrayValues(arr) {
  ...
  const iterValues = [...iter];

  const result = iterValues.filter((iter) => iter > 0);
  return result;
}
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const alignArr = arr.flat();
  const doubleItem = alignArr.map((item) => item * item);

  const sortedArr = doubleItem.filter((value) => value > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const lastElement = arr.slice(n + 1);

  const sum = lastElement.reduceRight(
    (accumulator, currentValue) => accumulator + currentValue
  );
...
function wordLengths(arr) {
  ...
  const result = arr.map((element) => (element = element.length));
  return result;
}
...
function flattenArray(arr) {
  ...
  const alignArr = arr.flat(Infinity);
...
function calculateTotalPrice(arr) {
  ...
  const result = arr.reduce((price, calc) => price + calc.price, 0);
  return result;
}
...
// НЕ ТА ВЕРСІЯ      НЕ ТА ВЕРСІЯ     НЕ ТА ВЕРСІЯ
